...
In the [main.yml](https://github.com/byrn-baker/Ansible-Workshop/blob/main/Cisco_Pod/roles/access_switch/add_vlan/tasks/main.yml) file we will use the playbook tasks structure. Roles simply replace tasks of a playbook.

Lets go over what we are doing:
* {% raw %}```name: Add new vlan to vlan database on {{ inventory_hostname }}```{% endraw %} - The name of the task appears in the Ansible console to let the operator know what is being performed in the background. "{{ }}" with Ansible anything between a double bracket is a variable and we can fill this in with anything available to Ansible like a hostname for example.
...
{% include embed/youtube.html id='46wQHUUbxrM' %}
...
{% include embed/youtube.html id='TLaH0f1ZUmM' %}
